...
function SubCategoryPage() {
  ...
  const getSubcategoryIdFromSlug = async (catId, slug) => {
    try {
      ...
      if (subCategoryList.length > 0) {
        const foundSub = subCategoryList.find(
          (sub) => 
            sub.slug?.toLowerCase() === slug?.toLowerCase() ||
            sub.name?.toLowerCase().replace(/\s+/g, '-').trim() === slug?.toLowerCase() ||
            String(sub.id) === String(slug)
        );
      ...
      return slug; // Return slug as fallback (might be numeric)
    } catch (error) {
  ...
  useEffect(() => {
    const fetchCategoryData = async () => {
      try {
        ...
        let resolvedCatId = categoryIdOrSlug;
        let categoryInfo = null;
        
        // Check if it's a numeric ID or a slug
        if (!/^\d+$/.test(categoryIdOrSlug)) {
          categoryInfo = await getCategoryIdFromSlug(categoryIdOrSlug);
          if (categoryInfo) {
        ...
        // Get subcategory ID from slug if needed
        let resolvedSubId = subcategoryIdOrSlug;
        if (!/^\d+$/.test(subcategoryIdOrSlug)) {
          resolvedSubId = await getSubcategoryIdFromSlug(resolvedCatId, subcategoryIdOrSlug);
        }
    ...
    if (categoryIdOrSlug && subcategoryIdOrSlug) {
      fetchCategoryData();
    } else {
      setLoading(false);
      setSubCategories([{ id: "all", name: "All", slug: "all", icon: "" }]);
    }
  }, [categoryIdOrSlug, subcategoryIdOrSlug]);
